...
class CatalogSerializer(BaseSerializer):
    id = serializers.IntegerField(min_value=1)
    nazev = serializers.CharField(source='name', max_length=255, required=False)
    obrazek_id = serializers.PrimaryKeyRelatedField(queryset=Image.objects.all(), source='image', required=False, allow_null=True)
    products_ids = serializers.PrimaryKeyRelatedField(source='products',many=True, queryset=Product.objects.all(), required=True)
    attributes_ids = serializers.PrimaryKeyRelatedField(source='attributes',many=True, queryset=Attribute.objects.all(), required=False, allow_null=True)
